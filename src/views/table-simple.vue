      ...
      <tbody>
        <tr v-for="(titem, tkey) in splitColModels" :key="tkey+'B'">
          <template v-for="(item, key) in titem.tdDatas">
            ...
            <td :colspan="item.colspan" :key="key+'D'" :class="item.cellClickEmit ? 'ss-simple-table-link' : ''" @click="handleCellClick(item, ditem)" v-html="formatData(item, ditem)"></td>
          </template>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  computed: {
    ...
    splitColModels () {
      ...
      let createCells = (v, key) => {
        v.colspan = parseInt(v.colspan) ? parseInt(v.colspan) : 1
        ...
        } else if (indexs > cellLen && _lineDataArray.length === 1) {
          // 判断最后一个的列数不等于总列数，进行补充列数处理
          createBlockCells(v)
          createCells(v, key)
        } else {
  ...
  methods: {
    formatData (item, data) {
      ...
      if (value && item.format) {
        value = dayjs(value).format(item.format)
      }
      if ((value || value === 0) && item.unit) {
        value += ` ${item.unit}`
      }
      if (!value && value !== 0 && item.field) {
        value = item.emptyTips ? item.emptyTips : '--'
      }

      if (item.linkMode) {
        let { showName, linkAttr, linkFixed } = item.linkMode
        value = showName || value
        value = linkAttr ? (data[linkAttr] ? `<a target="_blank" href="${data[linkAttr]}">${value}</a>` : value) : (linkFixed ? `<a target="_blank" href="${linkFixed}">${value}</a>` : value)
      }

      return value
    },
    handleCellClick (item, data) {
      if (!item.cellClickEmit) {
        return
      }
      this.$emit(item.cellClickEmit || `ss-simple-emit-${item.field}`, data)
    }
  }
}
</script>
...
.ss-simple-table .ss-simple-table-link{
  cursor: pointer;
